...
const OnboardingStep6Dashboard: React.FC<Props> = ({ onFinish }) => {
      ...
      <div className="relative flex h-full flex-col gap-4 overflow-hidden p-6">
        ...
        {visible &&
          [...Array(8)].map((_, i) => (
            <div
              key={i}
              className="confetti-piece pointer-events-none absolute text-lg"
              style={{
                left: `${10 + i * 11}%`,
                top: "-10px",
        ...
        {/* Header */}
        <div
          className={`flex items-start justify-between ${visible ? "s6-fade" : "opacity-0"}`}
        ...
        >
          ...
          <img
            src={opihello}
            alt="Opi"
            className="s6-robot w-25 drop-shadow-[0_8px_20px_rgba(124,92,252,0.5)]"
          />
        </div>

        <div
          className={`${visible ? "s6-fade" : "opacity-0"}`}
          style={{ animationDelay: "0.1s" }}
        >
          <h2 className="font-['Poppins'] text-[clamp(18px,2.5vw,45px)] font-extrabold text-white">
            You're all set! 🎉
          </h2>
          <p className="mt-0.5 text-[12px] text-gray-400">
            Your AI workforce is ready. Let's build something amazing!
          </p>
        </div>

        {/* Stats */}
        <div
          className={`grid grid-cols-3 gap-3 ${visible ? "s6-fade" : "opacity-0"}`}
        ...
        >
          {[
            ...
            <div
              key={i}
              className="glow-pulse rounded-2xl border border-white/8 bg-white/4 p-3"
            >
              <p className="text-[11px] text-gray-500">{s.label}</p>
              <div className="mt-1 flex items-end gap-1.5">
                <span className="count-in text-[22px] font-bold text-white">
              ...
              <button className="mt-1 text-[10px] font-semibold text-[#a78bfa] hover:text-[#c4b5fd] transition-colors">
                View all →
              </button>
            </div>
          ))}
        ...
        {/* Agent + Next steps */}
        <div
          className={`grid grid-cols-2 gap-3 ${visible ? "s6-fade" : "opacity-0"}`}
        ...
        >
          <div className="rounded-2xl border border-white/8 bg-white/4 p-3">
            ...
            <div className="flex items-center gap-2">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#7c5cfc]/25 text-lg">
                🤖
              </div>
              <div className="flex-1">
                ...
                <p className="text-[10px] text-gray-500">
                  Handles customer support 24/7 across all channels.
                </p>
              </div>
            </div>
            <button className="mt-2 w-full rounded-lg bg-[#7c5cfc]/20 py-1.5 text-[11px] font-semibold text-[#a78bfa] transition-all hover:bg-[#7c5cfc]/35">
              Open
            </button>
          </div>
          <div className="rounded-2xl border border-white/8 bg-white/4 p-3">
            ...
            {nextSteps.map((s, i) => (
              <div key={i} className="flex items-center gap-2 py-0.5">
                <div
                  className={`flex h-4 w-4 shrink-0 items-center justify-center rounded-full text-[9px] ${s.done ? "bg-[#7c5cfc] text-white" : "border border-white/20 text-transparent"}`}
                >
                  ✓
                </div>
                <span
                  className={`text-[11px] ${s.done ? "text-gray-300 line-through opacity-60" : "text-white"}`}
                >
                  {s.label}
                </span>
              </div>
          ...
          <div className="mb-3 flex items-center justify-between rounded-2xl border border-[#7c5cfc]/30 bg-[#7c5cfc]/10 px-4 py-3 cursor-pointer hover:bg-[#7c5cfc]/15 transition-colors">
            <div>
              ...
              <p className="text-[11px] text-gray-400">
                Need help getting started? Visit our template gallery.
              </p>
            </div>
