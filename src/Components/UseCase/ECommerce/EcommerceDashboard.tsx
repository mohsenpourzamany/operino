...
function RevenueChart({ visible }: { visible: boolean }) {
  ...
  const W = 500,
    H = 160;
  const rawData = [48000, 55000, 72000, 68000, 85000, 110000, 145000, 195000];
  const labels = [
    "May 10",
    "May 11",
    "May 12",
  ...
  const yLabels = ["$0", "$50K", "$100K", "$150K", "$200K"];
  const mn = 0,
    mx = 220000;
  const pts = rawData.map((v, i) => ({
    x: (i / (rawData.length - 1)) * (W - 20) + 10,
    ...
    <div>
      ...
      <div style={{ position: "relative" }}>
        ...
        <svg
          ...
          viewBox={`0 0 ${W} ${H + 10}`}
          preserveAspectRatio="none"
          style={{ overflow: "visible", paddingLeft: 36 }}
        >
          <defs>
            ...
            <clipPath id="revClip">
              <rect x="0" y="0" width={W * progress} height={H + 10} />
            </clipPath>
          </defs>
          {/* grid lines */}
          {yLabels.map((_, i) => {
          ...
          <path d={area} fill="url(#revGrad)" clipPath="url(#revClip)" />
          <path
            d={line}
            fill="none"
            stroke="#7c5cfc"
...
function TrafficDonut() {
  const segs = [
    { label: "Organic Search", pct: 40, color: "#7c5cfc" },
    { label: "Paid Ads", pct: 30, color: "#818cf8" },
    { label: "Social Media", pct: 15, color: "#a78bfa" },
    { label: "Direct", pct: 10, color: "#c4b5fd" },
    { label: "Others", pct: 5, color: "#e9d5ff" },
  ];
  const R = 44,
    CX = 52,
    CY = 52,
...
const insightData = [
  {
    icon: "🔥",
    title: "High Demand",
    body: "Wireless Headphones demand is increasing in your store.",
  },
  {
    icon: "🛒",
    title: "Cart Abandonment",
    body: "23% users left items in cart. AI can help recover them.",
  },
  {
    icon: "💰",
    title: "Price Optimization",
    body: "AI suggests a 5-10% price adjustment for 12 products.",
  },
];
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ width: 70 }}>
          <Sparkline data={sparkData} color="#7c5cfc" h={34} />
        </div>
      </div>
    </div>
  );
...
export default function EcommerceDashboard() {
  ...
  const spark1 = [40, 50, 44, 62, 58, 70, 66, 78];
  const spark2 = [30, 38, 44, 40, 52, 48, 58, 65];
  const spark3 = [55, 48, 60, 52, 64, 58, 70, 68];
  const spark4 = [28, 35, 30, 42, 38, 46, 44, 52];
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* COL 2: revenue chart + top products */}
          <div
            style={{
              display: "flex",
              flexDirection: "column",
      ...
      {/* responsive */}
      <style>{`@media(max-width:780px){.eco-dash-grid{grid-template-columns:1fr!important}}`}</style>
    </div>
  );
}
